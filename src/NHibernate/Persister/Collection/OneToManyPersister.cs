					...
					try
					{
						...
						foreach (object entry in entries)
						{
							if (await collection.NeedsUpdating(entry, i, ElementType))
							{
								...
								{
									deleteExpectation.VerifyOutcomeNonBatched(await session.Batcher.ExecuteNonQuery(st), st);
								}
								count++;
							}
							i++;
					...
					try
					{
						...
						foreach (object entry in entries)
						{
							if (await collection.NeedsUpdating(entry, i, ElementType))
							{
								...
								{
									insertExpectation.VerifyOutcomeNonBatched(await session.Batcher.ExecuteNonQuery(st), st);
								}
								count++;
							}
							i++;
